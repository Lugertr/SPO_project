...
export default class Parser {        // Помимо парсера в функции parseFormula() представлен ПОЛИЗ
    ...
    PostIfChek(): ExpressionNode {                                             // проверка на выполнения if перед else_if или else
        let I = this.pos;
        this.pos = this.pos - 3;
    ...
    parseGet(operatorLog: any): ExpressionNode {
        ...
        if (!thiskey) {thiskey = this.match(tokenTypesList.NUMBER);}
        console.log(thiskey);
        if (thiskey != null) {
    ...
    parseHash(): ExpressionNode {
        ...
        if (operatorLog != null) {
            ...
            if (!thiskey) {
                //this.pos-=1;
                thiskey = this.match(tokenTypesList.NUMBER);
            }
            if (thiskey != null) {
                let key = new KeyNode(thiskey);
                this.require(tokenTypesList.COMMA);
